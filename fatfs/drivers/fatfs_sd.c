...
)
{
	...
	return SDCARD_SPI_WriteRead(dat);
}
...
)
{
	...
	SDCARD_SPI_ReadMulti(buff, 0xFF, btr);
...
)
{
	...
	SDCARD_SPI_WriteMulti((uint8_t *)buff, btx);
}
#endif
...
)
{
	...
	DELAY_SetTime(wt * 1000);
	do {
		d = xchg_spi(0xFF);
	} while (d != 0xFF && DELAY_GetTime());	/* Wait for card goes ready or timeout */
	if (d == 0xFF) {
...
)
{
	...
	DELAY_SetTime(200000);
	do {							// Wait for DataStart token in timeout of 200ms 
		token = xchg_spi(0xFF);
		// This loop will take a time. Insert rot_rdq() here for multitask envilonment. 
	} while ((token == 0xFF) && DELAY_GetTime());
	if (token != 0xFE) {
...
DSTATUS TM_FATFS_SD_disk_initialize (void) {
	...
	if (send_cmd(CMD0, 0) == 1) {				/* Put the card SPI/Idle state */
		...
		DELAY_SetTime(1000000);
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2? */
			for (n = 0; n < 4; n++) ocr[n] = xchg_spi(0xFF);	/* Get 32 bit return value of R7 resp */
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) {				/* Is the card supports vcc of 2.7-3.6V? */
				while (DELAY_GetTime() && send_cmd(ACMD41, 1UL << 30)) ;	/* Wait for end of initialization with ACMD41(HCS) */
				if (DELAY_GetTime() && send_cmd(CMD58, 0) == 0) {		/* Check CCS bit in the OCR */
					for (n = 0; n < 4; n++) {
						ocr[n] = xchg_spi(0xFF);
					}
			...
			while (DELAY_GetTime() && send_cmd(cmd, 0));			/* Wait for end of initialization */
			if (DELAY_GetTime() || send_cmd(CMD16, 512) != 0) {	/* Set block length: 512 */
				ty = 0;
			}
		}
	}
